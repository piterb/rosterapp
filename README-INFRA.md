...
- Terraform apply 403 on Identity Platform/API Keys: ensure the Terraform admin SA has `roles/identitytoolkit.admin` and `roles/serviceusage.apiKeysAdmin` (granted in main Terraform). If creation still fails, re-run apply once bindings/APIs propagate.
